        ...
        const itemHtml = `
          <div class="card mb-4 p-3 border-0 bg-secondary-1 rounded-4 wow img-custom-anim-top">
              <div class="card-header border-0 bg-secondary-1">
                  <a class="${index === 0 ? '' : 'collapsed'} text-dark fw-bold" data-bs-toggle="collapse" href="#collapse${faq.id}">
                      ${faq.question}
                  </a>
              </div>
              <div id="collapse${faq.id}" class="collapse ${index === 0 ? 'show' : ''}" data-bs-parent="#faq-accordion">
  ...
  try {
    ...
    if (projects && projects.length > 0) {
      ...
      projects.forEach(proj => {
        const itemHtml = `
          <div class="col-lg-6 filter-item ${proj.category}">
              <div class="px-5 py-8">
                  <a href="portfolio-details.html?id=${proj.id}" class="shine-animate-item">
                      <span class="shine-animate mb-4">
                          <img class="w-100 rounded-3" src="${proj.imageUrl}" alt="${proj.title}" />
                      </span>
                      <span>
                          <span class="text-dark fw-medium mt-4 fs-2">${proj.title}</span><br />
                          <span class="fs-4 text-primary">${proj.services}</span>
                      </span>
                  </a>
              </div>
          </div>
    ...
    if (project) {
      ...
      if (specRow) {
        specRow.innerHTML = `
          <div class="col">
              <span>Client</span>
              <h6 class="fs-6 fw-semibold">${project.client || 'N/A'}</h6>
          </div>
          <div class="col">
              <span>Start</span>
              <h6 class="fs-6 fw-semibold">${project.startDate || 'N/A'}</h6>
          </div>
          <div class="col">
              <span>Complete</span>
              <h6 class="fs-6 fw-semibold">${project.completeDate || 'N/A'}</h6>
          </div>
          <div class="col">
              <span>Services</span>
              <h6 class="fs-6 fw-semibold">${project.services || 'N/A'}</h6>
          </div>
          <div class="col">
              <span>Website</span>
              <h6 class="fs-6 fw-semibold"><a href="https://${project.website}" target="_blank">${project.website || 'N/A'}</a></h6>
          </div>
        `;
      }
          ...
          swiperWrapper.insertAdjacentHTML('beforeend', `
            <div class="swiper-slide">
                <div class="position-relative">
                    <img class="rounded-4 w-100" src="${img}" alt="${project.title}" />
                </div>
            </div>
          `);
        });
